...
def anticor_algo_fixedAR(dat, train_size, window, holding, init_weight, cost):
    '''
    ...
    filter_res = KFfilter.fixed_filter(dat, train_size, np.eye(dim), np.eye(dim))
    #denoise price
    CAPR_df = pd.concat([np.log(filter_res.iloc[:,i+dim])-np.log(filter_res.iloc[:,i]) 
...
def anticor_algo_dynamicAR(dat, train_size, train_window, predict_size,
    ...
    dim = dat.shape[1]
    #calculate filtered result
    KF = KFfilter(dat, train_size, train_window, predict_size)
    KF.trainParam()
...
def batch_fixed (dat, train_size, window_lst, holding_lst, init_weight, cost):
    '''
    ...
    for window in window_lst:
        ...
        for holding in holding_lst:
            _, sharpe, down,_ = anticor_algo_fixedAR(dat, train_size, window, 
...
def batch_dynamic(dat, train_size, train_window, predict_size, 
    ...
    for window in window_lst:
        ...
        temp2 = []
        for holding in holding_lst:
            _, sharpe, down,_ = anticor_algo_dynamicAR(dat, train_size, 
